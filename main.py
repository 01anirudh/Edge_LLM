...
def smart_tokenizer_and_embedding_resize(
    ...
    """
    Resize tokenizer and embedding without averaging old embeddings (to avoid OOM).
    """
    ...
    # Optional: Initialize new embeddings with zeros (or skip completely)
    if num_new_tokens > 0:
        input_embeddings_data = model.get_input_embeddings().weight.data
        output_embeddings_data = model.get_output_embeddings().weight.data
        
        # Simple initialization (e.g., zeros or random)
        input_embeddings_data[-num_new_tokens:] = torch.zeros_like(input_embeddings_data[-num_new_tokens:])
        output_embeddings_data[-num_new_tokens:] = torch.zeros_like(output_embeddings_data[-num_new_tokens:])
